...
#ifndef WIN32
    {
        openlog("mqrestt", LOG_CONS | LOG_PID | LOG_NDELAY,
                    ...
                    const char *function, const char *formatstring, ...)
{
    ...
    if (type <= loglevel) {
        va_list args;
        char buffer[LOG_MSG_MAX_LENGTH]; // we dont want longer messages than
                                         // 256 chars
        int i = 0;
        char timestring[16];
        ...
        i = snprintf(buffer, LOG_MSG_MAX_LENGTH, "[%s][%s] (%s:%d,%s) ",
                     msg_head, print_time(timestring, sizeof(timestring)), file,
                     line, function);
        i = vsnprintf(buffer + i, LOG_MSG_MAX_LENGTH - i - 1, formatstring,
                      args);
        va_end(args);
