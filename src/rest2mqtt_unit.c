...
nfds_t
get_mhd_pollfds(struct MHD_Daemon *h, struct pollfd *pfds, nfds_t *count)
{
    int max = 0;
    fd_set rs, ws, es;
    ...
    for(int fd=0;fd<=max;fd++)
    {
        ...
        if(r_set || w_set || e_set)
        {
           ...
           if (r_set) pfds[pollfd_counter].events |= POLLIN;
           if (w_set) pfds[pollfd_counter].events |= POLLOUT;
           if (e_set) pfds[pollfd_counter].events |= POLLPRI;
        ...
        size_t *upload_data_size, void **con_cls)
{
    ...
    if (*upload_data_size)
    {
        INFO("GOT POST continuation, size %zd,  data: %s",*upload_data_size, upload_data);
        IncomingData *incoming = *con_cls;
        incoming->data = SAFEREALLOC(incoming->data, incoming->length + *upload_data_size); 
        incoming->data[incoming->length] = '\0';
        strncat(incoming->data + incoming->length, upload_data, *upload_data_size);
    ...
    else
    {
        ...
        free(incoming->data); free(incoming);
        return ret;
    }
}
